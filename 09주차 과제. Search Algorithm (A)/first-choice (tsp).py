...
def createProblem():
    ...
    numCities = int(infile.readline())  # The first line of infile is number of cities
    locations = []
...
def evaluate(current, p): ###
    ...
    for i in range(p[0]-1):
        ...
        cost += p[2][index1][index2]
    return cost


def randomMutant(current, p):   # Apply inversion
    while True:
        i, j = sorted([random.randrange(p[0])   # random.randrange(p[0])는 0 이상 30 미만인 하나의 난수.
                       for _ in range(2)])
        if i < j:
...
def inversion(current, i, j):  # Perform inversion
    curCopy = current[:]
    while i < j:   # inverse between i-th and j-th
...
def describeProblem(p):
    ...
    locations = p[1]
    for i in range(n):
        print("{0:>12}".format(str(locations[i])), end='')   # There are 12 spaces.
        if i % 5 == 4:   # Enter the white space when remainder is 4.
            print()
