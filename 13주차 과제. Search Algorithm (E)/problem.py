...
class Problem(Setup):
    ...
    def getSolution(self):   # 바깥에서 필요로 하는 값들을 반환하기 위해 accessor method를 만들어 줬다.
    ...
    def report(self):   # 코드의 중복을 없애기 위해 공통되는 것을 위로 올렸다.
        aType = self._aType
    ...
    def reportNumEvals(self):
        if 1 <= self._aType <= 4:   # 알고리즘 타입이 Hill climbing 계열인 것이다.
            print()
...
class Numeric(Problem):
    ...
    def evaluate(self, current):
        ## Evaluate the exproession of 'p' after assigning
    ...
    def report(self):
        ...
        self.reportNumEvals()   # aType이 Hill Climbing 계열인 경우에만 출력.
